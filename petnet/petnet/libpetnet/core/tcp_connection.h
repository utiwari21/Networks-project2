...
struct tcp_connection {
    ...
    /* **********************
     * Students fill in belows
     * **********************/

    //sending acknowledgement number
    uint32_t ack_num;
    ...
    //packet we are currently waiting for an ACK for, NULL if no packet is pending
    struct packet * pending_pkt;
    //timestamp of when we sent the pending packet, 0 if no packet is pending
